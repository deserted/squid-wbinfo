...
#
# Version history:
#   2013-10-25 Tim Allingham <tim@timallingham.net>
#               Added basic support for groups with spaces in name
...
while (<STDIN>) {
	...
	my ($ans, $thegroup, $group);
 	foreach $group (@groups) {
		 if (length($thegroup) == 0) {
                        $thegroup = $group;
												$thegroup =~ s/%([0-9a-fA-F][0-9a-fA-F])/pack("c",hex($1))/eg;
												$ans = &check($user, $thegroup);
                } else {
											$thegroup = $thegroup .' '. $group;
											$thegroup =~ s/%([0-9a-fA-F][0-9a-fA-F])/pack("c",hex($1))/eg;
											$ans = &check($user, $thegroup);
                }
								&debug("Answer is $ans for group $thegroup");
		 						last if $ans eq 'OK';
								if ($ans eq 'NOTMEM') {
									$thegroup = '';
								}
